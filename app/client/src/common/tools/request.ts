...
const defaultMessage: string = '请耐心等候，系统重启中......'
 ...
 function clearUserTraces () {
  Modal.confirm({
    ...
    onOk: () => {
      ...
      const navigate = store.getState().common.navigate
      if (navigate) {
        navigate('/login', {
          replace: true
        })
...
const request = new GotAxios({
  baseURL: typeof import.meta.env.VITE_REQUEST_URL === 'string' ? import.meta.env.VITE_REQUEST_URL: '',
}, {
  ...
  handleError(error: any) {
    ...
    switch (errorCode) {
      ...
      default:
        /** 控制台输出 */
        console.error(new Error(`请求发生错误 --> ${JSON.stringify(error, null, 2)}`))
        break;
    }
